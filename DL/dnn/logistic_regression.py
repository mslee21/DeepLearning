...
#     : MNIST image has 28*28 = 784 pixels
#     : 10 classes for digits 0 - 9 (one-hot vector)
X = tf.placeholder(tf.float32, [1,784], name='image') 
Y = tf.placeholder(tf.int32, [1,10], name='digit')
...
with tf.Session() as sess:
	...
	for i in range(n_epochs): # train the model n_epochs times
		total_loss = 0
		for j in range(mnist.train.num_examples):
			x, y = mnist.train.next_batch(1)
			_, l = sess.run([optimizer, loss], feed_dict={X: x, Y:y}) 
			total_loss += l
		print('Average loss epoch {0}: {1}'.format(i, total_loss/mnist.train.num_examples))
	...
	for i in range(mnist.test.num_examples):
		...
		accuracy_batch = sess.run(accuracy, feed_dict={X: x, Y: y}) 
